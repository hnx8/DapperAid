...
namespace DapperAid
{
    ...
    public partial class QueryRunner
    {
        ...
        public T SelectFirst<T>(Expression<Func<T, bool>>? where = null, Expression<Func<T, dynamic>>? targetColumns = null, string? otherClauses = null)
            where T : notnull
        {
            var parameters = new DynamicParameters();
            var sql = this.Builder.BuildSelect<T>(targetColumns)
                + this.Builder.BuildWhere(parameters, where)
                + this.Builder.BuildSelectOrderByEtc(targetColumns, otherClauses);
            return this.Connection.QueryFirst<T>(sql, parameters, this.Transaction, this.Timeout);
        }
        ...
        /// <typeparam name="TColumns">取得対象列にマッピングされた型</typeparam>
        /// <returns>取得したレコード（１件）</returns>
        public TColumns SelectFirst<TFrom, TColumns>(Expression<Func<TFrom, bool>>? where = null, string? otherClauses = null)
            where TFrom : notnull
            where TColumns : notnull
        {
            var parameters = new DynamicParameters();
            var sql = this.Builder.BuildSelect<TFrom, TColumns>()
                + this.Builder.BuildWhere(parameters, where)
                + this.Builder.BuildSelectOrderByEtc<TFrom, TColumns>(otherClauses);
            return this.Connection.QueryFirst<TColumns>(sql, parameters, this.Transaction, this.Timeout);
        }
